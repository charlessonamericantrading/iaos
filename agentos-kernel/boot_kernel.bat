...
if %ERRORLEVEL% EQU 0 (
    echo Launching QEMU...
    qemu-system-x86_64 -drive format=raw,file=target\disk-image\agentos-bios.img -serial stdio -netdev user,id=e1000net,guestfwd=tcp:10.0.2.100:9999-cmd:cat -device e1000,netdev=e1000net -netdev user,id=virtionet -device virtio-net-pci,netdev=virtionet
) else (
    echo [NOTE] QEMU is not installed in system PATH.
    echo To run the kernel in a virtual machine, execute:
    echo     qemu-system-x86_64 -drive format=raw,file=target\disk-image\agentos-bios.img -serial stdio -netdev user,id=e1000net,guestfwd=tcp:10.0.2.100:9999-cmd:cat -device e1000,netdev=e1000net -netdev user,id=virtionet -device virtio-net-pci,netdev=virtionet
)
