...
| Interactive shell | Real (small). The `AgentOS>` prompt is IRQ1-driven (not polled), buffers a real line (heap-backed `String`) with backspace, Left/Right cursor movement, and Up/Down command history (a real `Vec<String>`), and dispatches `help` / `ps` / `mem` / `uptime` / `date` / `lspci` / `disk` / `ls` / `cat` / `clear` against the live scheduler, KV-cache, heap, timer-tick, real-time-clock, and real disk/filesystem state - not canned output. Insert/backspace both act at the cursor, not just at the end of the line - typing "hep", pressing Left once, then typing "l" correctly produces "help", not "hepl" or "lhep". Verified in QEMU: that exact sequence dispatches the real `help` command (its actual output appears in the log), not an "Unknown command" line. |
...
| VirtIO-net / TCP/IP stack | `net::virtio_net` (the TCP/IP-facing driver) is still simulated - fixed MAC, packet counters, no real VirtIO MMIO/virtqueues. **But `net::e1000.rs` now reaches the real NIC**: finds it via PCI (vendor `0x8086`/device `0x100e`, confirmed present at 00:03.0 by real enumeration), reads its real BAR0, and reads back real MMIO registers - no new page-table mapping code needed, since this kernel already identity-maps all physical memory at a fixed offset for the VGA text buffer (`vga_buffer.rs`), and that mechanism turned out to cover PCI MMIO space too (confirmed by booting, not assumed). The MAC read from the real RAL0/RAH0 registers is `52:54:00:12:34:56` - QEMU's well-known default NIC MAC (the *exact* value this project's own `virtio_net.rs` had been using as a hardcoded placeholder, coincidentally - now read for real off actual hardware instead of typed in as a guess), identical across 3 repeated local boots. BAR0's *exact* address is NOT a similarly stable fact, found the hard way one CI run later: it read `0xfebc0000` locally but `0xfeb80000` on the CI runner's different QEMU version, from the identical source - SeaBIOS's PCI resource assignment isn't pinned the way the device's own emulated register contents are, so only the MAC/STATUS values (genuinely intrinsic to the e1000 model, unrelated to bus enumeration) are treated as portable facts, not the specific BAR0 value either QEMU happened to hand out. **This is a first step, not a full driver** - no TX/RX (descriptor rings) yet; that's real, substantial future work, deliberately not attempted in the same iteration as first proving MMIO access works at all. |
