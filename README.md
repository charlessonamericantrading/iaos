...
| Agent scheduler | Partial. Real priority-based process table, but no timer-driven preemption yet — picks a "next" PID without actually switching to it. |
| Context switch primitive | Real (proof-of-concept), **hand-written asm - flagged for extra human review**. `scheduler/context_switch.rs` is a genuine xv6-style cooperative switch: a `#[unsafe(naked)]` `switch_to` swaps RSP + the 6 callee-saved registers between two stacks. Verified in QEMU with the exact expected interleave (main → 3 worker iterations on its own heap-allocated stack → back to main, boot continues normally). Self-reviewed a second time (confirmed the stack-alignment math independently, confirmed the built-in `x86_64-unknown-none` target already disables the red zone, and fixed one real pointer-provenance issue - a stack pointer derived from `Box::as_ptr()` instead of `as_mut_ptr()`) - still deliberately left flagged for independent human review before more is built on it. Cooperative only - not yet wired to the timer IRQ (real preemption is separate follow-up work), and only proven for exactly one worker, not a general N-task scheduler yet. |
