...
| Partition table (MBR) | Real. `partition.rs` parses the 4 primary partition entries out of the boot sector already read via ATA. On our own disk image this finds two real bootable entries - a small internal region (the `bootloader` crate's own boot stages) and, more interestingly, **a genuine FAT32 partition** (type `0x0C`, 4096 sectors) - confirming the kernel binary really does live inside a real filesystem on disk, not just as raw bytes at a fixed offset. Next real step towards loading an actual file (a GGUF model, say): parse that FAT32 filesystem and read a file out of it by name - no filesystem code yet, this is still just the partition table pointing at where one lives. |
