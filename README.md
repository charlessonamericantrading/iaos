...
| Context switch primitive | Real (proof-of-concept), **hand-written asm - flagged for extra human review**. `scheduler/context_switch.rs` is a genuine xv6-style cooperative switch: a `#[unsafe(naked)]` `switch_to` swaps RSP + the 6 callee-saved registers between two stacks. Verified in QEMU with the exact expected interleave (main → 3 worker iterations on its own heap-allocated stack → back to main, boot continues normally). Deliberately cooperative only - not yet wired to the timer IRQ (that's real preemption, separate follow-up), and only proven for exactly one worker, not a general N-task scheduler yet. |
